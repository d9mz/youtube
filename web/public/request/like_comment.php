...
$videos_search = $__db->prepare(
    "SELECT * FROM comment_votes WHERE vote_type = :vote_type AND vote_target = :vote_target AND vote_from = :vote_from LIMIT 1"
);
...
if($request->error->message == "") {
    ...
    if($request->liked == true && $request->disliked == false) {
        if($request->like_type == "l") {
            $stmt = $__db->prepare("DELETE FROM comment_votes WHERE vote_target = ? AND vote_from = ? AND vote_type = ?");
            $stmt->execute(
                [
                    $request->like_target,
                    $request->like_from,
