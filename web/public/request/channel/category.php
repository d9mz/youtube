...
$request = (object) [
    "user_category"  => @$_POST['channel-type'],
...
if(!isset($_POST['channel-type'])) {
    $request->error->type    = 1;
    $request->error->message = "Category parameter is empty";
}
...
if($request->error->message == "") {
    $stmt = $__db->prepare("UPDATE users SET youtube_category = ? WHERE youtube_username = ?");
    $stmt->execute([
