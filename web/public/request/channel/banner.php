...
$ff = ["jpg", "png", "jpeg"];
print_r($_FILES);
$f = pathinfo($_FILES['banner']["name"], PATHINFO_EXTENSION);
...
if($request->error->message == "") {
    if(!exif_imagetype($_FILES['banner']["tmp_name"])) {
        $request->error->type    = 1;
    ...
    move_uploaded_file($_FILES['banner']["tmp_name"], "../../v/p/" . $fend);

    $stmt = $__db->prepare("UPDATE users SET youtube_banner = ? WHERE youtube_username = ?");
    $stmt->execute([
        $fend,
        $request->user_target,
    ]);
