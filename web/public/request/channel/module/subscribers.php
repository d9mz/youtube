...
if($request->error->message == "") {
	...
	if(isset($_POST['subscribers']) && $_POST['subscribers'] == "y") {
		$user["youtube_modules"]->subscribers_mod = true;
		...
		echo "LogicTrue";
	} else if(!isset($_POST['subscribers'])) {
		$user["youtube_modules"]->subscribers_mod = false;
		$user["youtube_modules"] = json_encode($user["youtube_modules"]);
		$stmt = $__db->prepare("UPDATE users SET youtube_modules = ? WHERE youtube_username = ?");
		$stmt->execute([
			$user["youtube_modules"],
