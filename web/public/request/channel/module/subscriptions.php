...
$request = (object) [
    "user_input"     => $_POST['subscriptions'],
...
if($request->error->message == "") {
	...
	if(isset($_POST['subscriptions']) && $_POST['subscriptions'] == "y") {
		$user["youtube_modules"]->subscriptions_mod = true;
		...
		$user["youtube_modules"]->subscriptions_mod = false;
		$user["youtube_modules"] = json_encode($user["youtube_modules"]);
		$stmt = $__db->prepare("UPDATE users SET youtube_modules = ? WHERE youtube_username = ?");
		$stmt->execute([
			$user["youtube_modules"],
